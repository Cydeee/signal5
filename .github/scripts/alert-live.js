 ...
 * Re-implements the entire A→H pipeline in Node.js,
 * then scores and sends a Telegram message if threshold is met.
 */
...
function score(raw) {
  const { long, short } = (() => {
    ...
    if (raw.dataD["15m"].bullVol > raw.dataD["15m"].bearVol) L++;
    else if (raw.dataD["15m"].bearVol > raw.dataD["15m"].bullVol) S++;
...
(async()=>{
  console.log("🔍 Starting standalone alert run");
  let data;
  try {
    data = await buildDashboardData();
  } catch(err) {
    console.error("❌ buildDashboardData failed:", err);
    process.exit(1);
  }
  console.log("▶ Dashboard data fetched");

  const { long, short } = score(data);
  ...
  if (long >= THRESHOLD || short >= THRESHOLD) {
    ...
    console.log("🚫 Score below threshold — nothing sent");
  }
})().catch(err=>{
