...
async function buildDashboardData(request) {
  ...
  try {
    ...
    // **new**: fetch your own JSON on Pages and pick BTC
    const liqJson = await safeJson(new URL("/data/totalLiquidations.json", request.url).href);
  ...
  try {
    ...
    result.dataE = {
      ...
      highRisk: stress>=5,
      components:{biasScore,levScore,volScore,divScore:0},
      source:'synthetic'
    };
  } catch (e) {
    result.dataE = null;
  ...
  try {
    ...
    result.dataF = {
      ...
      hhll20:{HH:+Math.max(...closes20).toFixed(2),LL:+Math.min(...closes20).toFixed(2)}
    };
  } catch (e) {
