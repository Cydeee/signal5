/* Netlify Edge Function  — serves /data  and  /data.json
   Pure fetch‑only code: compatible with Deno (Edge) + Node≥18
*/
async function buildDashboardData () {
  ...
  const rsi=(a,p)=>{ if(a.length<p+1) return 0; let up=0,dn=0; for(let i=1;i<=p;i++){const d=a[i]-a[i-1]; d>=0?up+=d:dn-=d;} let au=up/p,ad=dn/p;
    for(let i=p+1;i<a.length;i++){const d=a[i]-a[i-1]; au=(au*(p-1)+Math.max(d,0))/p; ad=(ad*(p-1)+Math.max(-d,0))/p;} return ad?100-100/(1+au/ad):100; };
  ...
  for(const tf of ['15m','1h','4h','1d']){
    try{
      ...
      out.dataA[tf]={ema50:+ema50.toFixed(2),ema200:+ema200.toFixed(2),rsi14:+rsi(c,14).toFixed(1),atrPct:+((atr(h,l,c,14)/last)*100).toFixed(2),macdHist:+macdHist.toFixed(2)};
  ...
  try{
    ...
    for(const k of min){ if(+k[0]<utc0) continue; const tp=(+k[2]+ +k[3]+ +k[4])/3, v=+k[5]; pv+=tp*v; vol+=v; prices.push(tp);}
...
export default async function handler(req){
  ...
  let payload; try{ payload=await buildDashboardData(); }catch(e){ return new Response('Internal error',{status:500}); }
  const hdr={'Access-Control-Allow-Origin':'*','Cache-Control':'public, max-age=0, must-revalidate'};
  if(wantJson){
    return new Response(JSON.stringify(payload),{headers:{...hdr,'Content-Type':'application/json; charset=utf-8'}});
  }
  return new Response(`<!doctype html><html><body><pre>${JSON.stringify(payload,null,2)}</pre></body></html>`,{headers:{...hdr,'Content-Type':'text/html; charset=utf-8'}});
}
