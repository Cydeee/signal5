...
// If this file is run directly (for local testing), write to disk:
if (require.main === module) {
  (async () => {
    const payload = await buildDashboardData();
    fs.mkdirSync('data', { recursive: true });
    fs.writeFileSync('data/totalLiquidations.json', JSON.stringify({ timestamp: Date.now(), data: payload.data || payload }, null, 2));
