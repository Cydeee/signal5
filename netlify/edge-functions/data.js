...
// 2025‑07‑27  — futures mirror fix + full file (no truncation)
...
async function binanceJson(path) {
  const hosts =
    path.startsWith("/fapi") || path.startsWith("/futures")
      ? FUTURE_HOSTS
      : SPOT_HOSTS;
  let last;
...
async function buildDashboardData() {
  ...
  try {
    ...
    for (const [lbl, hrs] of Object.entries(wins)) {
      ...
      let cvd = 0;
      trades.forEach(t => (cvd += t.m ? -(+t.q) : +t.q));
      out.dataD[lbl] = {
