...
// 2025‑07‑27  ➜ EU‑mirror strategy (api.binance.me  |  fapi.binance.me)
...
async function bJson(path, futures = false) {
  const hosts = futures ? FUT_HOSTS : SPOT_HOSTS;
  let lastErr;
...
async function buildDashboardData() {
  ...
  try {
    ...
    const oiΔ24  = (((+oiNow.openInterest - +oiHist[0].sumOpenInterest)/+oiHist[0].sumOpenInterest)*100).toFixed(1);
  ...
  try {
    ...
    const t24 = out.dataD["24h"].totalVol;
    const base={ "15m":t24/96, "1h":t24/24, "4h":t24/6 };
    out.dataD.relative={};
  ...
  /* H */
  try {
    const f0=(await safeJson("https://api.alternative.me/fng/?limit=1")).data?.[0]; if(!f0) throw new Error("FNG missing");
