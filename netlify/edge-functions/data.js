...
/* ─────────────── Shared builder (used by handler & CLI) ─────────────── */
export async function buildDashboardData () {
  ...
  for(const tf of ['15m','1h','4h','1d']){
    try{
      ...
      out.dataA[tf]={ema50:+ema50.toFixed(2),ema200:+ema200.toFixed(2),rsi14:+rsi(closes,14).toFixed(1),atrPct:+((atr(highs,lows,closes,14)/last)*100).toFixed(2),macdHist:+macdHist.toFixed(2)};
...
/* ─────────────────── Edge Function handler (default export) ─────────── */
export default async function handler(request) {
  // Decide output format based on path
  ...
  // simple HTML preview
  return new Response(`<!DOCTYPE html><html><body><pre>${JSON.stringify(payload,null,2)}</pre></body></html>`,{
    headers:{...headers,'Content-Type':'text/html; charset=utf-8'}
  });
}

/* ─────────────────── CLI: node data.js → write file ─────────────────── */
if (import.meta.url === `file://${process.argv[1]}`){
  (async()=>{
    const payload=await buildDashboardData();
    fs.mkdirSync('data',{recursive:true});
    fs.writeFileSync('data/totalLiquidations.json',JSON.stringify({timestamp:Date.now(),data:payload},null,2));
