...
// Blocks: A indicators | B derivatives+liquidations | C ROC | D volume+CVD | E stress (incl. liquidations) | F structure+VPVR | G macro | H sentiment
...
async function buildDashboardData() {
  ...
  // helper: fetch JSON with browser UA
  ...
  try {
    ...
    for (const [lbl,hrs] of Object.entries(windows)) {
      ...
      result.dataD.cvd[lbl] = +cvdVal.toFixed(2);
    }
  ...
  /* BLOCK E stress (incl. liquidation imbalance) */
  try {
    const b = Math.min(3, Math.abs(+result.dataB.fundingZ||0)),
          l = Math.max(0, (+result.dataB.oiDelta24h||0)/5),
          vFlag = result.dataD.relative["15m"],
          vScore = vFlag==="very high"?2: vFlag==="high"?1:0,
          liq = result.dataB.liquidations||{},
          imb = Math.abs((liq.long24h||0)-(liq.short24h||0)),
          liqScore = Math.min(2, imb/1e6),
          stress = b + l + vScore + liqScore;
    result.dataE = {
      stressIndex: +stress.toFixed(2),
      highRisk: stress>=5,
      components: { biasScore:b, levScore:l, volScore:vScore, liqScore },
      source:"synthetic"
    };
  } catch (e) {
    result.dataE = null;
    ...
    const vpvr = (bars) => {
      ...
      for (const b of bars) {
        const price = (+b[2]+ +b[3]+ +b[4])/3,
              vol   = +b[5],
              bin   = Math.round(price/100)*100;
        bkt[bin] = (bkt[bin]||0) + vol;
      }
      const [poc] = Object.entries(bkt).sort((a,b)=>b[1]-a[1])[0];
      return { poc: +poc, buckets: bkt };
    };
    result.dataF = { vpvr: { "4h": vpvr(bars4h), "1d": vpvr(bars1d), "1w": vpvr(bars1w) } };
  } catch (e) {
