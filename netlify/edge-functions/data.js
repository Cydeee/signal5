...
async function buildDashboardData(request) {
  ...
  const rsi=(a,p)=>{ if (a.length<p+1) return 0; let u=0,d=0; for (let i=1;i<=p;i++){const dff=a[i]-a[i-1]; dff>=0?u+=dff:d-=dff;} let au=u/p,ad=d/p; for(let i=p+1;i<a.length;i++){const dff=a[i]-a[i-1]; au=(au*(p-1)+Math.max(dff,0))/p; ad=(ad*(p-1)+Math.max(-dff,0))/p;} return ad?100-100/(1+au/ad):100; };
  ...
  try {
    ...
    // NEW: fetch your scraped JSON and attach BTC liquidations
  ...
  try {
    ...
    for (const [lbl,hrs] of Object.entries(windows)){
      let bull=0, bear=0, cut=now-hrs*3600000;
  ...
  try {
    ...
    result.dataE={ stressIndex:+stress.toFixed(2), highRisk:stress>=5, components:{biasScore,levScore,volScore,divScore:0}, source:"synthetic" };
  } catch (e) {
    result.dataE = null;
  ...
  try {
    ...
    for (const k of min1) if(+k[0]>=utc0){ const tp=(+k[2]+ +k[3]+ +k[4])/3; pv+=tp*k[5]; vol+=k[5]; prices.push(tp); }
    const vwap=pv/vol, sd=prices.length>1?Math.sqrt(prices.reduce((s,x)=>s+(x-vwap)**2,0)/prices.length):0;
    ...
    result.dataF={ pivot:{P:+P.toFixed(2),R1:+R1.toFixed(2),S1:+S1.toFixed(2)}, vwap:{value:+vwap.toFixed(2),band:+sd.toFixed(2)}, hhll20:{HH:+Math.max(...closes20).toFixed(2),LL:+Math.min(...closes20).toFixed(2)} };
  } catch (e) {
  ...
  /* BLOCK H sentiment */
  try {
    const fg=await safeJson("https://api.alternative.me/fng/?limit=1"), fgd=fg.data?.[0];
